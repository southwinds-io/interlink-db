...
DO $$
  BEGIN
    ...
    /*
      ...
      */
    CREATE OR REPLACE FUNCTION link_create_tag(root_item_key_param character varying,
    ...
    BEGIN
      ...
      tree_ids := (SELECT link_get_child_items(root_item_id));
      FOR loop_item IN
      ...
      tree_ids := (SELECT link_get_child_links(root_item_id));
      FOR loop_item IN
    ...
    /*
      ...
      */
    CREATE OR REPLACE FUNCTION link_update_tag(root_item_key_param character varying,
    ...
    BEGIN
      ...
      SELECT link_get_update_status(current_version, local_version_param, rows_affected > 0) INTO result;
      RETURN QUERY SELECT result;
    END;
    $BODY$;

    /*
      link_delete_tag: deletes the specified tag.
     */
    CREATE OR REPLACE FUNCTION link_delete_tag(root_item_key_param character varying,
    ...
    ALTER FUNCTION link_delete_tag(character varying, character varying)
      OWNER TO interlink;

    /*
      link_get_tree_content(root_item_key_param, label_param): inspects the tag hstores for information
        about a specific tag items and links and retrieve a set of ids and versions for them.
     */
    CREATE OR REPLACE FUNCTION link_get_tree_content(root_item_key_param character varying, label_param character varying)
      RETURNS TABLE
              (
                id      text,
                version text,
    ...
    ALTER FUNCTION link_get_tree_content(character varying, character varying)
      OWNER TO interlink;

    /*
      link_get_tree_items(root_item_key_param, label_param): gets a list of all the items that are part
        of a tag tree for a specific parent item and a label.
    */
    CREATE OR REPLACE FUNCTION link_get_tree_items(root_item_key_param character varying,
    ...
    BEGIN
      RETURN QUERY
        ...
        FROM link_get_tree_content(root_item_key_param, label_param) s
    ...
    ALTER FUNCTION link_get_tree_items(character varying, character varying)
      OWNER TO interlink;

    /*
      link_get_tree_links(root_item_key_param, label_param): gets a list of all the links that are part
        of a tag tree for a specific parent item and a label.
     */
    CREATE OR REPLACE FUNCTION link_get_tree_links(root_item_key_param character varying, label_param character varying)
      RETURNS TABLE
              (
                operation      character,
                changed        timestamp with time zone,
    ...
    ALTER FUNCTION link_get_tree_links(character varying, character varying)
      OWNER TO interlink;

    /*
      link_get_item_tags(root_item_key_param): gets a list of tags for a specified items that
        is the parent for the tag tree.
     */
    CREATE OR REPLACE FUNCTION link_get_item_tags(root_item_key_param character varying)
      RETURNS TABLE
              (
                id            integer,
    ...
    ALTER FUNCTION link_get_item_tags(character varying)
      OWNER TO interlink;
