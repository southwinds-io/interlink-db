...
DO $$
  BEGIN
     ...
     */
    CREATE OR REPLACE FUNCTION link_is_super_admin(
     ...
     */
    CREATE OR REPLACE FUNCTION link_can_manage_partition(
      role_key_param character varying[]
    )
      RETURNS VOID
      LANGUAGE 'plpgsql'
    ...
    ALTER FUNCTION link_can_manage_partition(character varying[])
      OWNER TO interlink;
     ...
     */
    CREATE OR REPLACE FUNCTION link_get_update_status(
    ...
    ALTER FUNCTION link_get_update_status(bigint, bigint, boolean)
      OWNER TO interlink;
     ...
     */
    CREATE OR REPLACE FUNCTION link_check_item_attr(item_type_key character varying, attributes hstore)
        RETURNS VOID
        LANGUAGE 'plpgsql'
        COST 100
        STABLE
    ...
    ALTER FUNCTION link_check_item_attr(character varying, hstore)
        OWNER TO interlink;
     ...
     */
    CREATE OR REPLACE FUNCTION link_check_link_attr(link_type_key character varying, attributes hstore)
      RETURNS VOID
      LANGUAGE 'plpgsql'
      COST 100
      STABLE
    ...
    ALTER FUNCTION link_check_link_attr(character varying, hstore)
      OWNER TO interlink;

    /*
      select link_check_link(link_type_key_param, start_item_type_key_param, end_item_type_key_param):
        checks that a link of a given type is valid (i.e. can be used to join to items of given types
        in a particular direction)
     */
    CREATE OR REPLACE FUNCTION link_check_link(link_type_key_param character varying,
    ...
    ALTER FUNCTION link_check_link(character varying, character varying, character varying)
      OWNER TO interlink;
